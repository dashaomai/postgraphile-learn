...
COMMENT ON COLUMN learn.card.review_after IS 'The next time the card should be reviewed.';
COMMENT ON COLUMN learn.card.spacing IS 'The current spacing in days between reviews';
COMMENT ON COLUMN learn.card.ease_factor IS 'The ease at which the answer was last remembered.';
COMMENT ON COLUMN learn.card.seq IS 'Controls the order in which to review cards.';
COMMENT ON COLUMN learn.card.archived IS 'Indicates if the card is active or not.';
COMMENT ON COLUMN learn.card.person_id IS 'The creator/owner of this card.';
COMMENT ON COLUMN learn.card.created_at IS 'The time this card was created.';
